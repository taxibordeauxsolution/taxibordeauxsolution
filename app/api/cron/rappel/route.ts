...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const r of rappels24h) {
      ...
      if (ok) { await Reservation.findByIdAndUpdate(r._id, { rappel24h: true }); sent++ }
    }
    ...
    for (const r of rappels1h) {
      ...
      if (ok) { await Reservation.findByIdAndUpdate(r._id, { rappel1h: true }); sent++ }
    }
...
async function sendRappel(r: any, delai: string, token: string, chatId: string) {
  ...
  const text = [
    ...
    '', `📋 ${r.reservationId}`, `👤 ${r.customer.name}`, `📞 ${r.customer.phone}`,
    '', `🕐 ${datePickup} à ${heurePickup}`, `📍 ${r.trip.from}`, `➡️ ${r.trip.to}`,
    `📏 ${r.trip.distance?.toFixed(1)} km`, `💰 ${prix}`,
    r.notes ? `📝 ${r.notes}` : '',
  ].filter(Boolean).join('\n')
