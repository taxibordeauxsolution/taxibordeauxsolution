...
export default function ForfaitForm({ initial, token, onSaved, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 overflow-y-auto" role="dialog" aria-modal="true">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-blue-100 dark:border-slate-700 p-6 sm:p-8 space-y-6 w-full max-w-2xl my-4 animate-modal-in">
        ...
        <div>
          ...
          <input type="text" value={form.nom} onChange={e => setField('nom', e.target.value)} placeholder="Ex: Gare Saint-Jean ↔ Aéroport Mérignac"
            className="w-full border-2 border-gray-300 dark:border-slate-600 dark:bg-slate-700 text-gray-900 bg-white dark:text-slate-100 dark:bg-transparent rounded-xl px-4 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1" />
        </div>
              ...
              <div key={z} className={`border-2 rounded-2xl p-4 space-y-3 ${z === 'A' ? 'border-blue-200 bg-blue-50/30 dark:border-blue-900/40 dark:bg-blue-900/20' : 'border-red-200 bg-red-50/30 dark:border-red-900/40 dark:bg-red-900/20'}`}>
                ...
                <div>
                  ...
                  <input ref={z === 'A' ? acRefA : acRefB} type="text" defaultValue={form[pt].adresse} placeholder="Tapez une adresse..."
                    className="w-full border-2 border-gray-300 dark:border-slate-600 dark:bg-slate-700 text-gray-900 bg-white dark:text-slate-100 dark:bg-transparent rounded-xl px-3 py-2 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 text-sm" />
                </div>
                <div className="flex items-center gap-2">
